...
describe("Export", function() {
  it("no bookmarks", function() {
    ...
    expect('1.1').toBe(response.version);
    expect('Resting WebExtension').toBe(response.creator.name);
    expect('1.0').toBe(response.creator.version);
  });
   ...
   it("folder with one bookmark", function() {
    ...
    expect('Resting WebExtension').toBe(response.creator.name);
    expect('1.0').toBe(response.creator.version);
    expect(2).toBe(response.entries.length);
    expect("FOLDER").toBe(response.entries[0]._name);
    expect(true).toBe(response.entries[0]._isFolder);
    expect("7e1a4356-fede-41c3-b764-5024c8f17188").toBe(response.entries[0]._id);
    expect("2019-05-28T04:36:38.346Z").toBe(response.entries[0]._created);
    expect('my-bookmark').toBe(response.entries[1]._name);
    expect('www.google.it').toBe(response.entries[1].url)
    expect('PUT').toBe(response.entries[1].method);
    expect(2).toBe(response.entries[1].headers.length);
    expect(true).toBe(response.entries[1].headers[1]._enabled);
    expect("application/json").toBe(response.entries[1].postData.mimeType);
    expect('{\n\"field\" : \"value\" \n}').toBe(response.entries[1].postData.text);
    expect('gino').toBe(response.entries[1]._authentication.username);
  });
  ...
  it("bookmark post request urlencoded", function() {
    ...
    expect('Resting WebExtension').toBe(response.creator.name);
    expect('1.0').toBe(response.creator.version);
    expect(1).toBe(response.entries.length);
    expect("book").toBe(response.entries[0]._name);
    expect(false).toBe(response.entries[0]._isFolder);
    expect("9406075c-e754-4d9b-a264-cb6b9f4fd9ea").toBe(response.entries[0]._id);
    expect("2019-06-11T04:49:40.308Z").toBe(response.entries[0]._created);
    expect('test.local').toBe(response.entries[0].url)
    expect('POST').toBe(response.entries[0].method);
    expect(0).toBe(response.entries[0].headers.length);
    expect("application/x-www-form-urlencoded").toBe(response.entries[0].postData.mimeType);
    expect(response.entries[0].postData.text).toBeUndefined();
    expect(response.entries[0].postData.params.length).toBe(2);
    expect('').toBe(response.entries[0]._authentication.username);
  });
  ...
  it("bookmark post request form-data", function() {
    ...
    expect(response.creator.name).toBe('Resting WebExtension');
    expect(response.creator.version).toBe('1.0');
    expect(response.entries.length).toBe(1);
    expect(response.entries[0]._name).toBe("test");
    expect(response.entries[0]._isFolder).toBe(false);
    expect(response.entries[0]._id).toBe("2cf18425-2f1c-4491-9706-930f6c679f9b");
    expect(response.entries[0]._created).toBe("2019-06-14T04:29:43.764Z");
    expect(response.entries[0].url).toBe('google.com')
    expect(response.entries[0].method).toBe('POST');
    expect(response.entries[0].headers.length).toBe(0);
    expect(response.entries[0].postData.mimeType).toBe("multipart/form-data");
    expect(response.entries[0].postData.text).toBeUndefined();
    expect(response.entries[0].postData.params.length).toBe(3);
    expect(response.entries[0]._authentication.username).toBe('');
  });
  ...
  it("contexts", function() {
    ...
    expect(response._contexts.length).toBe(3);
    expect(response._contexts[0].name).toBe('default');
    expect(response._contexts[0].variables.length).toBe(1);
    expect(response._contexts[0].variables[0]).toEqual({name:'def', value:'1111', enabled:true});
    expect(response._contexts[1].name).toBe('dev');
    expect(response._contexts[1].variables.length).toBe(3);
    expect(response._contexts[2].name).toBe('prod');
    expect(response._contexts[2].variables.length).toBe(2);
...
describe("Import", function() {
  ...
  it("bookmark of request multipart body", function() {
    const sampleExport = `{"version":"1.1","creator":{"name":"Resting WebExtension","version":"1.0"},"entries":[{"_name":"test","_isFolder":false,"_id":"2cf18425-2f1c-4491-9706-930f6c679f9b","_created":"2019-06-14T04:29:43.764Z","headerSize":-1,"bodySize":-1,"url":"google.com","method":"POST","headers":[],"queryString":[],"postData":{"mimeType":"multipart/form-data","params":[{"name":"p1","value":"v1","_enabled":true},{"name":"p2","value":"v2","_enabled":true},{"name":"p3","value":"v3","_enabled":true}]},"_authentication":{"username":"","password":""}}],"_contexts":[]}`;
    const bookmarkProvider = makeBookmarkProvider(mockStorageProvider);
   ...
   it("bookmark of request JSON body", function() {
    ...
    //const bookmarkProvider = makeBookmarkProvider(mockStorageProvider);
    //const response = bookmarkProvider.exportObj(JSON.parse(d));
